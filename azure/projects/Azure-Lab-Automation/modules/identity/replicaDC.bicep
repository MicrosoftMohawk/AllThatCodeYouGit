...
resource cse 'Microsoft.Compute/virtualMachines/extensions@2024-03-01' = {
  ...
  properties: {
    ...
    protectedSettings: {
      commandToExecute: 'powershell -ExecutionPolicy Bypass -Command "Start-Transcript -Path C:\\WindowsTemp\\ReplicaDC.log -Append; $nic = Get-NetAdapter | Where-Object Status -eq Up | Select-Object -First 1; Set-DnsClientServerAddress -InterfaceIndex $nic.ifIndex -ServerAddresses ${q}${primaryDcIp}${q}; Write-Output ${q}DNS set to ${primaryDcIp}${q}; $maxWait = 600; $waited = 0; while ($waited -lt $maxWait) { try { Resolve-DnsName ${q}${domainName}${q} -DnsOnly -ErrorAction Stop | Out-Null; Write-Output ${q}Domain DNS resolved${q}; break } catch { Start-Sleep 15; $waited += 15 } }; if ($waited -ge $maxWait) { throw ${q}Domain not reachable within 600s${q} }; Install-WindowsFeature AD-Domain-Services -IncludeManagementTools; $pw = ConvertTo-SecureString ${q}${adminPassword}${q} -AsPlainText -Force; $dsrm = ConvertTo-SecureString ${q}${dsrmPassword}${q} -AsPlainText -Force; $cred = New-Object System.Management.Automation.PSCredential(${q}${netbiosName}\\${adminUsername}${q}, $pw); Install-ADDSDomainController -DomainName ${q}${domainName}${q} -Credential $cred -SafeModeAdministratorPassword $dsrm -InstallDns:$true -NoRebootOnCompletion:$false -Force:$true; Stop-Transcript"'
    }
  }
}
